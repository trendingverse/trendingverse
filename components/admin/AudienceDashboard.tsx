...
export function AudienceDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'geo' | 'leads'>('overview')
  ...
  async function sendEmailReport() {
    ...
    try {
      ...
      const html = `
        ...
        <h3>Recent Leads</h3>
        <table border="1" cellpadding="6" style="border-collapse:collapse;width:100%">
          <tr style="background:#f3f4f6"><th>Email</th><th>Name</th><th>City</th><th>Source</th></tr>
          ${(data?.recentLeads || []).slice(0, 20).map(l => `<tr><td>${l.email}</td><td>${l.name || '—'}</td><td>${l.city || '—'}</td><td>${l.source_site}</td></tr>`).join('')}
        </table>`
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 p-1 bg-ink-100 rounded-xl w-fit">
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key as 'overview' | 'geo' | 'leads')}
      ...
      {activeTab === 'geo' && (
        <div className="space-y-5">
          ...
          {/* India map placeholder with city bubbles */}
          <div className="card p-5">
            ...
            <p className="text-xs text-ink-300 mt-4">Bubble size = relative visitor count. Hover for details.</p>
          </div>
        </div>
      )}
        ...
        <div className="card overflow-hidden">
          ...
          {recentLeads.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-ink-500">No leads yet — subscribe bar will start collecting emails from publisher sites</p>
            </div>
          ) : (
