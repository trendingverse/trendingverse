...
export function PublisherAdAssignment() {
  ...
  async function fetchSitesForPublisher(publisherId: string) {
    const res = await fetch(`/api/sites?publisher_id=${publisherId}`)
  ...
  const selectedPublisherAssignments = assignments.filter(a => !form.publisher_id || a.publisher_id === form.publisher_id)
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select className="input" value={form.publisher_id}
              onChange={e => { setForm({...form, publisher_id: e.target.value, site_id: ''}); fetchSitesForPublisher(e.target.value) }}>
          ...
          <div>
            ...
            <select className="input" value={form.site_id} onChange={e => setForm({...form, site_id: e.target.value})}>
              ...
              {sites.map(s => <option key={s.id} value={s.id}>{s.name} — {s.site_url}</option>)}
            </select>
          </div>
          <div>
      ...
      <div className="card overflow-hidden">
        <div className="p-4 border-b border-ink-100 flex items-center justify-between">
          <h4 className="font-medium text-ink-900 text-sm">Current Assignments ({selectedPublisherAssignments.length})</h4>
          <select className="input text-xs w-48" value={form.publisher_id}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {selectedPublisherAssignments.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-ink-300 text-sm">No assignments yet</td></tr>
            )}
            {selectedPublisherAssignments.map((a, i) => (
