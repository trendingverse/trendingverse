...
// Nav links verified — uncertain ones left at best guess
...
const NAV = [
  ...
  { group: 'Content', items: [
    ...
    { label:'Paste & Enrich',   href:'/admin/paste-enrich',             icon:'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
  ...
  { group: 'AI & SEO', items: [
    ...
    { label:'Author & Category',href:'/admin/author-category',          icon:'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
  ]},
  { group: 'Monetization', items: [
    ...
    { label:'Direct Campaigns', href:'/admin/monetization/direct-campaigns', icon:'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z' },
  ...
  { group: 'System', items: [
    ...
    { label:'View Site',        href:'/', external:true,               icon:'M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14' },
  ]},
]

function Icon({ d }: { d: string }) {
  return <svg width={15} height={15} fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" aria-hidden><path d={d}/></svg>
}

type Item = { label: string; href: string; icon: string; external?: boolean }

function NavItem({ item, active }: { item: Item; active: boolean }) {
  const base = 'group relative flex items-center gap-2.5 px-3 py-[7px] rounded-lg text-[13px] font-medium transition-all duration-150'
  const cls  = active ? 'bg-red-500/10 text-white' : 'text-slate-500 hover:text-slate-200 hover:bg-white/[0.04]'
  const inner = (
    <span className={`${base} ${cls}`}>
      {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-4 bg-red-500 rounded-r-full"/>}
      <span className={`flex-shrink-0 ${active?'text-red-400':'text-slate-600 group-hover:text-slate-400'}`}><Icon d={item.icon}/></span>
      <span className="truncate">{item.label}</span>
      {item.external && <svg className="ml-auto flex-shrink-0 text-slate-700" width={10} height={10} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6m0 0v6m0-6L10 14" strokeLinecap="round" strokeLinejoin="round"/></svg>}
    </span>
  )
  if (item.external) return <a href={item.href} target="_blank" rel="noopener noreferrer">{inner}</a>
...
export function AdminSidebar() {
  ...
  useEffect(() => {
    fetch('/api/mediation/revenue-summary')
      ...
      .then(d => { if(d?.today_revenue!=null) setRev(`$${Number(d.today_revenue).toFixed(4)}`) })
      .catch(()=>{})
  }, [])
  ...
  return (
    <aside className="flex flex-col h-screen flex-shrink-0" style={{width:224,background:'#070c18',borderRight:'1px solid #131f33'}}>

      {/* Logo */}
      <div className="flex items-center gap-3 px-4 h-[56px] flex-shrink-0" style={{borderBottom:'1px solid #131f33'}}>
        <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 text-white font-bold text-xs" style={{background:'linear-gradient(135deg,#e63030 0%,#991b1b 100%)'}}>TV</div>
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-4" style={{scrollbarWidth:'none'}}>
        {NAV.map(s=>(
          <div key={s.group}>
            <p className="px-3 mb-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-700">{s.group}</p>
            <div className="space-y-0.5">
              {s.items.map(item=><NavItem key={item.href} item={item} active={isActive(item.href)}/>)}
            </div>
          </div>
        ))}
      </nav>
      ...
      <div className="flex-shrink-0 mx-3 mb-3 rounded-lg p-3" style={{background:'#0d1726',border:'1px solid #131f33'}}>
        ...
        <p className="text-base font-mono font-bold text-emerald-400">{rev ?? '—'}</p>
        <Link href="/admin/revenue" className="text-[11px] text-slate-600 hover:text-red-400 transition-colors mt-1 block">View earnings →</Link>
      </div>

      {/* User */}
      <div className="flex items-center gap-2.5 px-4 py-3 flex-shrink-0" style={{borderTop:'1px solid #131f33'}}>
        <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-white text-xs font-bold" style={{background:'linear-gradient(135deg,#dc2626,#7c3aed)'}}>Y</div>
        ...
        <Link href="/api/auth/signout" className="flex-shrink-0 text-slate-700 hover:text-slate-400 transition-colors" title="Sign out">
          <svg width={14} height={14} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden><path d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </Link>
      </div>
    </aside>
  )
