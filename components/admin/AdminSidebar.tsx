...
/* ─── Navigation — paths verified against actual repo routes ───── */
const NAV = [
  {
    group: 'Overview',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Direct Campaigns', href: '/admin/direct-campaigns',          icon: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z' },
      { label: 'ads.txt',          href: '/admin/ads-txt',                   icon: 'M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4' },
    ],
  },
  {
...
export function AdminSidebar() {
  ...
  useEffect(() => {
    fetch('/api/mediation/revenue-summary')
      ...
      .then(d => { if (d?.today_revenue != null) setRevenue(`$${Number(d.today_revenue).toFixed(4)}`) })
      .catch(() => {})
  }, [])
  ...
  return (
    <aside className="flex flex-col h-screen flex-shrink-0"
      ...
      <div className="flex-shrink-0 mx-3 mb-3 rounded-lg p-3" style={{ background:'#0d1726', border:'1px solid #131f33' }}>
        ...
        <p className="text-base font-mono font-bold text-emerald-400">{revenue ?? '—'}</p>
