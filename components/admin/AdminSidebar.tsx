...
const NAV = [
  ...
  { group: 'AI & SEO', items: [
    ...
    { label: 'Authors',        href: '/admin/author-fix',                    d: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
  ]},
...
export function AdminSidebar({ isAdmin = false, isAdvertiser = false }: { isAdmin?: boolean; isAdvertiser?: boolean }) {
  ...
  const nav = isAdvertiser ? ADVERTISER_NAV : isAdmin ? [...NAV, ADMIN_GROUP] : NAV
  ...
  const SidebarInner = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className={`flex items-center h-14 border-b border-ink-100 dark:border-dark-border flex-shrink-0 ${collapsed ? 'justify-center px-3' : 'justify-between px-4'}`}>
        {!collapsed && (
          <Link href="/" className="font-display font-bold text-[17px] tracking-tight text-ink-950 dark:text-white">
            Trending<span className="text-accent">Verse</span>
          </Link>
        )}
        <button onClick={() => setCollapsed(!collapsed)}
          className="hidden lg:flex w-8 h-8 rounded-lg items-center justify-center text-ink-400 hover:text-ink-700 hover:bg-ink-100 dark:text-slate-600 dark:hover:text-slate-300 dark:hover:bg-dark-50 transition-all"
          title={collapsed ? 'Expand' : 'Collapse'}>
          <Icon d={collapsed
            ? 'M13 5l7 7-7 7M5 5l7 7-7 7'
            : 'M11 19l-7-7 7-7M19 19l-7-7 7-7'} size={14} />
        </button>
      </div>

      {/* Nav */}
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-5" style={{ scrollbarWidth: 'none' }}>
        {nav.map(section => (
          <div key={section.group}>
            {!collapsed && (
              <p className="text-[10px] font-bold uppercase tracking-[0.1em] px-3 mb-1.5 text-ink-300 dark:text-slate-700">
                {section.group}
              </p>
            )}
                ...
                return (
                  <Link key={item.href} href={item.href}
                    ...
                    className={[
                      ...
                      active
                        ? 'bg-accent/8 text-accent dark:bg-accent/10 dark:text-red-400'
                        : 'text-ink-500 hover:text-ink-900 hover:bg-ink-50 dark:text-slate-500 dark:hover:text-slate-100 dark:hover:bg-dark-50',
                    ].join(' ')}>
                    <span className={`flex-shrink-0 transition-colors ${active ? 'text-accent dark:text-red-400' : 'text-ink-400 group-hover:text-ink-600 dark:text-slate-600 dark:group-hover:text-slate-400'}`}>
                      <Icon d={item.d} />
                    </span>
                    {!collapsed && <span className="truncate">{item.label}</span>}
                    {!collapsed && active && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-accent dark:bg-red-400 flex-shrink-0" />}
                  </Link>
                )
              })}
            </div>
      ...
      {/* View Site */}
      <div className="flex-shrink-0 px-2 py-2 border-t border-ink-100 dark:border-dark-border">
        <a href="/" target="_blank" rel="noopener noreferrer"
          title={collapsed ? 'View Site' : undefined}
          className={[
            'flex items-center gap-2.5 rounded-lg text-xs text-ink-400 hover:text-ink-700 hover:bg-ink-50 dark:text-slate-600 dark:hover:text-slate-300 dark:hover:bg-dark-50 transition-all',
            collapsed ? 'justify-center px-2 py-2.5' : 'px-3 py-2',
          ].join(' ')}>
  ...
  return (
    <>
      ...
      <button onClick={() => setMobileOpen(!mobileOpen)}
        className="lg:hidden fixed top-3.5 left-3.5 z-50 w-9 h-9 rounded-xl bg-white dark:bg-dark-100 border border-ink-100 dark:border-dark-border shadow-sm flex items-center justify-center text-ink-700 dark:text-slate-300">
      ...
      {/* Mobile sidebar */}
      <aside className={`lg:hidden fixed top-0 left-0 h-full w-60 z-50 bg-white dark:bg-dark-100 border-r border-ink-100 dark:border-dark-border shadow-2xl transform transition-transform duration-200 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarInner />
      </aside>

      {/* Desktop sidebar */}
      <aside className={`hidden lg:flex flex-col bg-white dark:bg-dark-100 border-r border-ink-100 dark:border-dark-border flex-shrink-0 transition-all duration-200 ${collapsed ? 'w-14' : 'w-56'}`}>
        <SidebarInner />
      </aside>
    </>
  )
