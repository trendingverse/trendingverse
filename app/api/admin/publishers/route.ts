...
export async function GET() {
  ...
  // Get all user profiles with article counts
  const { data: profiles } = await admin
    .from('user_profiles')
    .select('id, full_name, plan, subscription_status, articles_used_today, created_at')
  ...
  const { data: sites } = await admin
    .from('sites')
    .select('user_id, name, site_url, articles_count')
