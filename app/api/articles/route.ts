...
export async function POST(req: NextRequest) {
  ...
  // Use the SAME algorithmic scorer the editor uses, so the stored score
  // always matches what the editor displays. Previously this imported a
  // different (older) computeSeoScore from lib/utils which produced a
  // different number — causing the list to show 80 while the editor showed 90.
  const scoreResult = computeSeoScore({
    title: rest.seo_title || rest.title || '',
    content,
    metaDescription: rest.meta_description || '',
