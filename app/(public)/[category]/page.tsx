...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (['admin', 'login', 'api', 'search', 'article'].includes(slug)) {
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  if (['admin', 'login', 'api', 'search', 'article', '_next'].includes(slug)) {
    notFound()
  }
  ...
  const { data: arts, count: total } = await supabase
    .from('articles')
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mb-8">
        ...
        {cat.description && (
          <p className="text-ink-500 ml-6">{cat.description}</p>
        )}
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="grid sm:grid-cols-2 gap-5">
            {((arts as Article[]) || []).map((a) => (
              <ArticleCard key={a.id} article={a} variant="grid" />
            ))}
          </div>
          {arts?.length === 0 && (
            <p className="text-center text-ink-300 py-16">
