...
export default async function ArticlesPage() {
  ...
  const draftArticles = (articles || []).filter(a => a.status === 'draft' || a.status === 'published')

  return (
    <div className="space-y-6">
      <WordPressPublisher articles={draftArticles} />
      <ArticleManager articles={articles || []} categories={categories || []} />
    </div>
  )
}
