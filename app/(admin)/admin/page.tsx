...
// Server component — no 'use client', no next/dynamic
...
const fmtDate  = (iso: string) => {
  try { return new Date(iso).toLocaleDateString('en-IN', { month:'short', day:'numeric' }) } catch { return iso }
}
...
function SectionHead({ title, action }: { title: string; action?: { label: string; href: string } }) {
  return (
    <div className="flex items-center justify-between mb-4">
      ...
      {action && (
        <Link href={action.href} className="text-[12px] font-medium text-red-500 hover:text-red-400 transition-colors">
          {action.label} →
        </Link>
      )}
    </div>
...
async function sq<T>(fn: () => Promise<{ data: T | null; error: unknown }>): Promise<T | null> {
  try {
    const { data, error } = await fn()
    if (error) { console.error('[Dashboard]', error); return null }
    return data
  } catch (e) {
...
export default async function AdminDashboard() {
  ...
  const [totalArt, drafts, todayPub, revRows, articles, adNetworks, sites, recentRev] = await Promise.all([
    sq(() => supabase.from('articles').select('id', { count: 'exact', head: true })),
    sq(() => supabase.from('articles').select('id', { count: 'exact', head: true }).eq('status', 'draft')),
    sq(() => supabase.from('articles').select('id', { count: 'exact', head: true }).eq('status','published').gte('published_at', todayISO + 'T00:00:00Z')),
    sq<RevRow[]>(() =>
  ...
  const totalCount = (totalArt as CountResult | null)?.count ?? 0
  const draftCount = (drafts   as CountResult | null)?.count ?? 0
  const pubCount   = (todayPub as CountResult | null)?.count ?? 0
    ...
    <div className="space-y-6 pb-8" style={{ color: C.text }}>
      ...
      <div className="flex items-start justify-between pt-1">
        ...
        <div className="flex items-center gap-2">
          <Link href="/admin/ai-writer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-medium transition-colors" style={{ background: C.elevated, border: `1px solid ${C.border}`, color: C.muted }}>
            ✦ AI Writer
          </Link>
          <Link href="/admin/articles/new" className="px-4 py-1.5 rounded-lg text-[13px] font-semibold text-white" style={{ background: '#dc2626' }}>
            + New Article
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Stat label="Total articles"    value={String(totalCount)}  sub={`${draftCount} drafts · ${pubCount} published today`}     color="text-amber-400"   />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        {(recentRev ?? []).length > 0 ? (
          ...
          <Card className="p-5 flex items-center justify-center">
            <p className="text-[13px] text-center" style={{ color: C.dim }}>
              Revenue activity will appear here<br />once ad networks are connected.
            </p>
          </Card>
        )}
      </div>
