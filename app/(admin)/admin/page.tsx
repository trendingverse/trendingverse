...
/* ─── Tokens ─────────────────────────────────────────────────────── */
const C = {
  surface:   '#0e1726', elevated:  '#131f33',
  border:    '#1a2840', borderDim: '#111e30',
  text:      '#dde4f0', muted:     '#6b82a8', dim: '#2d3f58',
}
...
function SH({ title, action }: { title:string; action?: { label:string; href:string } }) {
  return (
...
async function sq<T = any>(fn: () => any): Promise<T | null> {
  try {
    const res = await fn()
    if (res?.error) { console.error('[sq]', JSON.stringify(res.error)); return null }
    return (res?.data ?? null) as T | null
  } catch (e) { console.error('[sq threw]', e); return null }
}
async function sqCount(fn: () => any): Promise<number> {
  try {
    const res = await fn()
    if (res?.error) { console.error('[sqCount]', JSON.stringify(res.error)); return 0 }
    return typeof res?.count === 'number' ? res.count : 0
  } catch (e) { console.error('[sqCount threw]', e); return 0 }
}

/* ─── Normalize a partner_revenue row ───────────────────────────── */
// Handles any column name variation in the DB
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function normRev(r: any): RevRow {
  return {
    date:         r.date ?? r.created_at?.slice?.(0,10) ?? '',
    revenue_usd:  r.revenue_usd   ?? r.revenue   ?? r.earnings         ?? r.amount      ?? 0,
    impressions:  r.impressions   ?? r.impr       ?? r.impression_count ?? r.views        ?? 0,
    network_name: r.network_name  ?? r.network    ?? r.partner_name     ?? r.ad_network  ?? 'Unknown',
    site_url:     r.site_url      ?? r.site       ?? r.domain           ?? '',
  }
}

type RevRow  = { date:string; revenue_usd:number; impressions:number; network_name:string; site_url:string }
...
export default async function AdminDashboard() {
  // Regular client for articles (respects RLS)
  const supabase = await createClient()

  // Service role client for revenue data (bypasses RLS — admin only)
  ...
  /* ── Parallel queries ──────────────────────────────────────────── */
  const [totalArt, drafts, todayPub, rawRevRows, rawRecentRev, articles, adNetworks, sites] = await Promise.all([
    // Articles — regular client (user table, RLS fine)
    ...
    // Revenue — service role (bypasses RLS on partner_revenue)
    sq(() =>
      svc.from('partner_revenue')
        .select('*')                          // select * so column names don't matter
    ...
    // Networks + sites — service role
    sq<Network[]>(() => svc.from('ad_networks').select('id, name, is_active, priority').order('priority', { ascending:true })),
    sq<Site[]>(()    => svc.from('sites').select('id, name, site_url, is_active').limit(6)),
  ])

  /* ── Normalize revenue rows ─────────────────────────────────────── */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const rows:      RevRow[] = (rawRevRows   as any[] | null ?? []).map(normRev)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const recentRev: RevRow[] = (rawRecentRev as any[] | null ?? []).map(normRev)

  /* ── Aggregations ───────────────────────────────────────────────── */
  const todayRev  = rows.filter(r => r.date === todayISO).reduce((s,r) => s+r.revenue_usd, 0)
  const weekRev   = rows.filter(r => r.date >= d7.toISOString().slice(0,10)).reduce((s,r) => s+r.revenue_usd, 0)
  const monthRev  = rows.reduce((s,r) => s+r.revenue_usd, 0)
  const monthImpr = rows.reduce((s,r) => s+r.impressions, 0)
  const todayImpr = rows.filter(r => r.date === todayISO).reduce((s,r) => s+r.impressions, 0)
  ...
  const chartData = chartDays.map(date => ({
    date:        date.slice(5),
    revenue:     rows.filter(r=>r.date===date).reduce((s,r)=>s+r.revenue_usd, 0),
    impressions: rows.filter(r=>r.date===date).reduce((s,r)=>s+r.impressions, 0),
  }))

  /* ── Network breakdown ──────────────────────────────────────────── */
  const netMap: Record<string,number> = {}
  rows.forEach(r => { netMap[r.network_name] = (netMap[r.network_name]??0) + r.revenue_usd })
    ...
    <div className="space-y-6 pb-8" style={{ color:C.text }}>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 p-5">
          <SH title="Revenue — 14 Days" action={{ label:'Full report', href:'/admin/revenue' }} />
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        {recentRev.length > 0 ? (
              ...
              <tbody>
                {recentRev.map((r,i) => (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors" style={{ borderBottom:`1px solid ${C.borderDim}` }}>
                    ...
                    <td className="py-2 px-4 text-right font-mono" style={{ color:C.muted }}>{fmtK(r.impressions)}</td>
                    <td className="py-2 px-4 text-right font-mono font-semibold text-emerald-400">{fmtMoney(r.revenue_usd)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <Card className="p-5 flex items-center justify-center">
            <p className="text-[13px] text-center" style={{ color:C.dim }}>Revenue activity will appear here<br/>once networks start reporting.</p>
          </Card>
        )}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 overflow-hidden">
          ...
          {(articles ?? []).length === 0 ? (
            <p className="text-[13px] text-center py-8" style={{ color:C.dim }}>No articles. <Link href="/admin/articles/new" className="text-red-500">Create one →</Link></p>
          ) : (
            <div>
              {(articles ?? []).map(a => {
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <SH title="Ad Networks" action={{ label:'Manage', href:'/admin/monetization/ad-networks' }} />
            <div className="space-y-2.5">
              {(adNetworks ?? []).length === 0 ? (
                ...
                <div key={n.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2"><Dot on={n.is_active} /><span className="text-[13px]" style={{ color:n.is_active?C.text:C.dim }}>{n.name}</span></div>
          ...
          <Card className="p-4">
            <SH title="Publisher Sites" action={{ label:'Manage', href:'/admin/monetization' }} />
            <div className="space-y-2.5">
              {(sites ?? []).length === 0 ? (
                <p className="text-[12px] text-center py-3" style={{ color:C.dim }}>No sites yet</p>
              ) : (sites ?? []).map(s => (
                <div key={s.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2 min-w-0"><Dot on={s.is_active} /><span className="text-[13px] truncate" style={{ color:s.is_active?C.text:C.dim }}>{s.name||s.site_url}</span></div>
