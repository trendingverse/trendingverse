...
async function sq(fn: () => any): Promise<number> {
  try { const r = await fn(); return r?.error ? 0 : (r?.count ?? 0) } catch { return 0 }
}
async function qd<T>(fn: () => any): Promise<T[]> {
...
export default async function AdminDashboard() {
  ...
  const [total, published, drafts, todayPub, articles] = await Promise.all([
    ...
    qd<any>(() => supabase.from('articles').select('id,title,status,published_at,category_name').order('created_at', { ascending: false }).limit(8)),
  ])
  ...
  const kpis = [
    ...
    { icon:'🚀', label:'PUBLISHED TODAY',    value: todayPub,  bg:'rgba(239,68,68,0.12)',  color:'#f87171' },
  ]
            ...
            {actions.map(a => (
              <Link key={a.href} href={a.href}
                style={{
                  ...
                  fontSize:13, fontWeight:600, textDecoration:'none', transition:'all 0.1s', marginBottom:2,
                  background: a.accent ? 'var(--accent)' : 'transparent',
                  color: a.accent ? '#fff' : 'var(--txt-2)',
                }}
